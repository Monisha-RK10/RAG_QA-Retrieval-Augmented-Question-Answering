...
def load_llm(model_name: str = "google/flan-t5-large"):
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(                                         # Loads the actual LLM weights
        model_name, device_map="auto", torch_dtype="auto"                                  # Automatically places the model on GPU if available, else CPU, chooses optimal precision (float16 on GPU, float32 on CPU).
    )
